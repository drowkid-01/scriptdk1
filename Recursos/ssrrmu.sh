...
msg -bar
    echo -e " CHUKK-SCRIPT by @drowkid01"
...
msg -bar
	stty erase '^H' && read -p "(Predeterminado: @drowkid01):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user="@drowkid01"
...
msg -bar
	pxrtx=$(shuf -i 1-9829 -n 1)
	stty erase '^H' && echo -ne "(\e[1;33mPREDETERMINADO: \e[92m$pxrtx) "
	read -p ": " ssr_port
	[[ -z "$ssr_port" ]] && ssr_port=${pxrtx}
...
Set_config_password(){
msg -bar
	echo -e "\e[92m 3) Por favor ingrese una contraseña para el Usuario"
msg -bar
	stty erase '^H' && read -p "(Predeterminado: drowkid01):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="drowkid01"
